...
def _build_model(args, device: torch.device) -> RDTForCausalLM:
    ...
    if isinstance(raw_rdt, dict):
        rdt_cfg = rdt_config_from_dict(raw_rdt)
...
def print_script_cer(tag: str, rep) -> None:
    ...
    Buckets with ``n_ref == 0`` are already omitted from ``rep.script_cer``
    (see :func:`Model.ocr.metrics.script_bucket_cer`), so nothing to skip
    here.
    """
