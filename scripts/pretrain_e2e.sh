#!/usr/bin/env bash
# End-to-end text-pretraining pipeline for the DoL-OCR language backbone.
#
...
#
# Real run (corpus roots must be explicit absolute paths outside this repo):
#   MN_CORPUS="/mnt/datasets/traditional-mongolian" \
#   CN_CORPUS="/mnt/datasets/chinese" \
#   WIKI_DUMP="/mnt/datasets/wikipedia" \
#   scripts/pretrain_e2e.sh
#
# Key environment knobs:
#   WORK            working directory for all intermediate artifacts
#   MN_CORPUS       optional external traditional Mongolian corpus directory
#                   (UTF-16 .txt, recursive); no repository-local default
#   CN_CORPUS       optional external Chinese corpus directory; no
...
#   WIKI_DUMP       optional external Wikimedia parquet dump directory
#                   ({DATE}.{lang}/*.parquet); empty uses HF streaming
...
if [ "$SMOKE" = "1" ]; then
    ...
    PRECISION="fp32"
    MAMBA="naive"
    # Keep the advertised local self-test independent of multiprocessing
    # shared-memory support (not available in some macOS/sandbox runtimes).
    SMOKE_TRAIN_ARGS=(--num-workers 0)
fi

# Honor an externally provided WORK; otherwise use the mode-specific default.
WORK="${WORK:-$WORK_DEFAULT}"

validate_external_corpus_path() {
    local name="$1"
    local value="$2"
    [ -z "$value" ] && return 0
    case "$value" in
        /*) ;;
        *)
            echo "ERROR: $name must be an absolute external path: $value" >&2
            exit 2
            ;;
    esac
    case "$value" in
        "$ROOT"|"$ROOT"/*)
            echo "ERROR: $name must be outside the DoL-OCR repository: $value" >&2
            exit 2
            ;;
    esac
}
...
if [ "$SMOKE" != "1" ]; then
    validate_external_corpus_path MN_CORPUS "$MN_CORPUS"
    validate_external_corpus_path CN_CORPUS "$CN_CORPUS"
    validate_external_corpus_path WIKI_DUMP "$WIKI_DUMP"
fi
...
def _has_local(lang):
    return bool(dump) and bool(
        glob.glob(os.path.join(dump, f"{date}.{lang}", "*.parquet"))
    )
...
JSONL
    ...
    for lang in $WIKI_LANGS; do
        if [ -n "$WIKI_DUMP" ] && \
                ls "$WIKI_DUMP/$WIKI_DATE.$lang"/*.parquet >/dev/null 2>&1; then
