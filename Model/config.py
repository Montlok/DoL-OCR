...
class RDTConfig:
    ...
    # interleaved per step). "two_stage" is the production DoL-1.2 core: a
    # pure-Mamba encoding stage followed by recurrent attention refinement.
    ...
    def _check_core(self) -> None:
        if self.core_type not in {"interleaved", "two_stage"}:
            raise ValueError(
                "core_type must be 'interleaved' or 'two_stage', "
...
# The reviewed DoL-1.2-OCR metadata was produced before the unused
# segmented/MoL/ACT experiments were removed.  Keep compatibility at the
# serialization boundary, not in the runtime config or model.  Every legacy
# field must have the exact inactive value from the reviewed checkpoint; an
# actually enabled removed branch fails closed.
_REMOVED_INACTIVE_RDT_CONFIG_FIELDS: dict[str, object] = {
    "act_max_steps": 32,
    "act_ponder_cost": 0.01,
    "act_threshold": 0.99,
    "kv_share_budget": 0,
    "mol_experts": 4,
    "mol_rank": 8,
...
def rdt_config_from_dict(payload: Mapping[str, object]) -> RDTConfig:
    """Restore an RDT config while rejecting removed active experiments.

    New checkpoints contain only current :class:`RDTConfig` fields.  The
    reviewed DoL-1.2-OCR source metadata also contains the exact inactive
    legacy values above; those keys are authenticated by the release lock,
    verified here, and discarded before construction.
    """

    if not isinstance(payload, Mapping):
        raise TypeError("RDT config payload must be a mapping")
    values = dict(payload)
    if values.get("core_type") == "segmented":
        raise ValueError("segmented RDT checkpoints are not supported")
    for field, expected in _REMOVED_INACTIVE_RDT_CONFIG_FIELDS.items():
        if field not in values:
            continue
        actual = values.pop(field)
        if type(actual) is not type(expected) or actual != expected:
            raise ValueError(
                f"removed RDT config field {field!r} is not the reviewed "
                f"inactive value {expected!r}"
            )
    return RDTConfig(**values)
