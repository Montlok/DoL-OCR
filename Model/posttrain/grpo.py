...
(here verifiable) reward, and form advantages within that group.  The OCR path
uses a single synchronous on-policy update with a k3 KL penalty toward a frozen
reference.  PPO clipping remains an explicit legacy option for callers that
actually retain an older behavior policy; OCR disables it because clipping a
ratio that is evaluated before the one and only update is not a trust region.

Rigor rules enforced here:
- Advantages are formed **within each prompt group**. OCR defaults to the
  K2.5-style mean baseline without standard-deviation amplification; the
  legacy standardized form remains explicit for text experiments.
...
def _generate_sequences_with_behavior(
    model,
    input_ids: torch.Tensor,
    **kwargs,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Generate and retain exact sampling log-probs at token positions."""

    behavior_steps: list[torch.Tensor] = []
    ...
    def _capture(
        ...
        behavior_steps.append(token_logp.detach())

    if "on_sample" in kwargs:
        raise ValueError("on_sample is owned by GRPO behavior capture")
    ...
    completion_len = sequences.shape[1] - input_ids.shape[1]
    if len(behavior_steps) != completion_len:
        raise RuntimeError(
            "generation did not return one behavior log-prob per completion "
            f"step: {len(behavior_steps)} != {completion_len}"
        )
    ...
    if behavior_steps:
        behavior[:, input_ids.shape[1] :] = torch.stack(behavior_steps, dim=1)
    return sequences, behavior
...
def group_normalized_advantages(
    ...
    """Backward-compatible standardized group advantages."""
...
def group_relative_advantages(
    ...
    *,
    mode: str = "centered",
    min_reward_spread: float = 0.0,
    eps: float = 1e-6,
) -> torch.Tensor:
    """Build group-relative advantages without amplifying tiny OCR deltas.

    Args:
        rewards: ``[N]`` where ``N = num_prompts * group_size``, group-contiguous.
        group_size: responses sampled per prompt.
        mode: ``"centered"`` uses ``reward - group_mean``; ``"standardized"``
            additionally divides by group standard deviation.
        min_reward_spread: groups whose ``max-min`` is at or below this value
    ...
    if mode not in {"centered", "standardized"}:
        raise ValueError("advantage mode must be centered or standardized")
    if not math.isfinite(min_reward_spread) or min_reward_spread < 0:
    ...
    if mode == "standardized":
        ...
        adv = adv / (std + eps)
    spread = groups.max(dim=1, keepdim=True).values - groups.min(
        dim=1, keepdim=True
    ).values
    adv = torch.where(spread > min_reward_spread, adv, torch.zeros_like(adv))
...
class GRPOConfig:
    ...
    min_reward_spread: float = 0.0
    max_behavior_log_ratio: float | None = None

    def __post_init__(self) -> None:
        if self.clip_eps is not None and not 0.0 < self.clip_eps < 1.0:
        ...
        if (
            not math.isfinite(self.min_reward_spread)
            or self.min_reward_spread < 0
        ):
            raise ValueError("min_reward_spread must be finite and non-negative")
        if self.max_behavior_log_ratio is not None and (
            not math.isfinite(self.max_behavior_log_ratio)
            or self.max_behavior_log_ratio <= 0
        ):
            raise ValueError("max_behavior_log_ratio must be positive when set")
...
def _is_single_omvt_payload(payload: PixelValues | None) -> bool:
    if not isinstance(payload, Mapping):
        return False
    batch_sizes = [
        int(value.shape[0])
        for key, value in payload.items()
        if not key.endswith("_bbox") and value.ndim > 0
    ]
    return bool(batch_sizes) and all(size == 1 for size in batch_sizes)
...
def grpo_loss(
    ...
    *,
    enforce_behavior_gate: bool = True,
    ...
    active_log_ratio = raw_log_ratio.detach().abs()[mask.bool()]
    zero_metric = raw_log_ratio.new_zeros((), dtype=torch.float32)
    behavior_log_ratio_abs_max_tensor = (
        active_log_ratio.max() if active_log_ratio.numel() else zero_metric
    )
    behavior_log_ratio_abs_p99_tensor = (
        torch.quantile(active_log_ratio.float(), 0.99)
    ...
    metric_names = (
        ...
        "kl",
        "ratio_mean",
        "behavior_log_ratio_abs_max",
        "behavior_log_ratio_abs_p99",
        "adv_mean",
        "adv_abs_mean",
        "adv_abs_max",
        "clip_frac",
        "numeric_clip_frac",
        "kl_numeric_clip_frac",
        "completion_tokens",
    )
    metric_tensors = (
        loss,
        _masked_sequence_mean(pg),
        _masked_sequence_mean(kl),
        _masked_sequence_mean(ratio),
        behavior_log_ratio_abs_max_tensor,
        behavior_log_ratio_abs_p99_tensor,
        advantages.mean(),
        advantages.abs().mean(),
        advantages.abs().max(),
        _masked_sequence_mean(clipped),
        _masked_sequence_mean(numeric_clipped),
        _masked_sequence_mean(kl_numeric_clipped),
        mask.sum(),
    )
    metric_values = torch.stack(
        [value.detach().to(dtype=torch.float32) for value in metric_tensors]
    ).cpu().tolist()
    metrics = dict(zip(metric_names, metric_values, strict=True))
    behavior_log_ratio_abs_max = metrics["behavior_log_ratio_abs_max"]
    if (
        enforce_behavior_gate
        and cfg.max_behavior_log_ratio is not None
        and behavior_log_ratio_abs_max > cfg.max_behavior_log_ratio
    ):
        raise RuntimeError(
            "rollout/scoring policy mismatch: completion-token "
            f"|log_ratio|max={behavior_log_ratio_abs_max:.6g} exceeds "
            f"{cfg.max_behavior_log_ratio:.6g}"
        )
    return loss, metrics
...
def _sample_group_with_behavior(
    model: RDTForCausalLM,
    prompt_ids: torch.Tensor,
    cfg: GRPOConfig,
    *,
    ...
    pad_id: int | None,
    pixel_values: PixelValues | None,
    morphology_track_table: torch.Tensor | None,
    pixel_repeats: int = 1,
    position_contract: str | None = None,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Internal rollout path with exact behavior log-prob capture."""

    prompt_length = prompt_ids.shape[0]
    batch = prompt_ids.unsqueeze(0).expand(cfg.group_size, -1).contiguous()
    sequences, behavior = _generate_sequences_with_behavior(
        model,
        batch,
        max_new_tokens=cfg.max_new_tokens,
        temperature=cfg.temperature,
    ...
    return (
        sequences,
        _completion_action_mask(sequences, prompt_length, eos_id),
        behavior,
    )
...
def _grpo_compute_loss_impl(
    ...
    score the group with ``reward_fn`` (decoded via ``decode``), form
    group-relative advantages, verify the exact sampling/scoring log-ratio,
    and accumulate the policy surrogate + KL toward ``reference`` (frozen).
    The optimizer step is left to the caller.
    ...
    if position_contract is not None and morphology_track_table is not None:
        raise ValueError(
    ...
    unique_omvt_payloads = all(
        _is_single_omvt_payload(payload) for payload in pixel_values
    )
    pixel_repeats = cfg.group_size if unique_omvt_payloads else 1
    grouped_payloads = (
        list(pixel_values)
        if unique_omvt_payloads
        else [
            repeat_pixel_values(payload, cfg.group_size)
            for payload in pixel_values
        ]
    )
    prompt_lengths = {int(prompt.shape[0]) for prompt in prompts}
    completion_start = (
        next(iter(prompt_lengths)) - 1 if len(prompt_lengths) == 1 else None
    )
    rollout_groups: (
    ...
    if len(prompt_lengths) == 1:
        ...
        rollout_sequences, rollout_behavior = _generate_sequences_with_behavior(
            policy,
            prompt_batch,
            max_new_tokens=cfg.max_new_tokens,
            temperature=cfg.temperature,
    ...
    behavior_logp_groups: list[torch.Tensor] = []
    ...
    for idx, prompt in enumerate(prompts):
        ...
        if rollout_groups is not None:
            seqs, mask, behavior_logp = rollout_groups[idx]
        else:
            seqs, mask, behavior_logp = _sample_group_with_behavior(
                policy,
                prompt,
                cfg,
                eos_id=eos_id,
        ...
        reward_spread = rewards.max() - rewards.min()
        ...
        agg["reward_spread"] = agg.get("reward_spread", 0.0) + float(
            reward_spread
        )
        agg["active_group"] = agg.get("active_group", 0.0) + float(
            reward_spread > cfg.min_reward_spread
        )
        ...
        behavior_logp_groups.append(behavior_logp)
    ...
    behavior_logp = torch.cat(
        [_right_pad(group, 0.0) for group in behavior_logp_groups],
        dim=0,
    )
    advantages = torch.cat(advantage_groups, dim=0)
    score_pixels = combine_pixel_values(grouped_pixel_payloads)

    # One fresh synchronous update is taken per rollout batch. ``behavior_logp``
    # was captured from the exact distribution used by torch.multinomial;
    # comparing it with this differentiable full-forward score makes any cache,
    # precision, or rollout-service drift observable before the update.
    ...
    total, loss_metrics = grpo_loss(
        ...
        cfg=cfg,
        enforce_behavior_gate=enforce_behavior_gate,
    )
    n = len(prompts)
...
def grpo_compute_loss(
    ...
    *,
    ...
    ``generate()`` samples in evaluation mode.  The exact sampled-action
    log-probabilities are captured during generation and compared with a fresh
    differentiable full-forward score.  Evaluation mode removes dropout drift
    without disabling autograd; the caller's original module modes are restored
    before returning.
    """
